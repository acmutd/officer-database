...
export function Navbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 py-2 bg-black">
            ...
            <button
                onClick={() => logout()}
                className="absolute right-8 flex items-center justify-center shrink-0 h-10 w-10 rounded-full  text-white transition-all duration-200 hover:bg-white/25 focus:outline-none focus:ring-2 focus:ring-white/40 focus:ring-offset-2 focus:ring-offset-transparent"
                aria-label="Log Out"
                title="Log Out"
            >
